...
export default function LessonPage({
  ...
  const initialCode = useMemo(() => {
    ...
    return `// Welcome to Solana Quest!\n// Write your code here...\n\nimport { Connection, PublicKey, LAMPORTS_PER_SOL } from "@solana/web3.js";\n\nconst connection = new Connection("https://api.devnet.solana.com");\n\nasync function main() {\n  const publicKey = new PublicKey("11111111111111111111111111111111");\n  const balance = await connection.getBalance(publicKey);\n  console.log(\`Balance: \${balance / LAMPORTS_PER_SOL} SOL\`);\n}\n\nmain();`;
  }, [lesson]);

  // Set initial code once
  useState(() => {
    if (!code) setCode(initialCode);
  });
  ...
  // ============================================
  // Evaluates code challenges by checking:
  // 1. Required patterns (keywords, function calls, imports)
  // 2. Expected output patterns from test cases
  // 3. Syntax checks (balanced braces, no empty functions)
  // This runs client-side — no server needed.

  const evaluateCode = useCallback(
    (
      userCode: string,
      challenge: NonNullable<typeof lesson.challenge>
    ): { name: string; passed: boolean; message?: string }[] => {
      const normalizedCode = userCode.replace(/\s+/g, ' ').trim();

      return challenge.testCases.map((tc) => {
        // Parse expected output as evaluation criteria
        // Format: "pattern1|pattern2|..." — all must be present
        // Or "REGEX:pattern" for regex matching
        const expected = tc.expectedOutput.trim();

        if (!expected) {
          // No expected output defined — check code was modified from starter
          const starterNormalized = challenge.starterCode.replace(/\s+/g, ' ').trim();
          const modified = normalizedCode !== starterNormalized && normalizedCode.length > starterNormalized.length;
          return {
            name: tc.name,
            passed: modified,
            message: modified ? 'Code modified from starter' : 'Modify the starter code to complete this test',
          };
        }
        ...
        if (expected.startsWith('REGEX:')) {
          ...
          try {
            ...
            const passed = regex.test(userCode);
            return {
              name: tc.name,
              passed,
              message: passed ? 'Pattern matched' : 'Expected pattern not found in code',
            };
          } catch {
            return { name: tc.name, passed: false, message: 'Invalid test pattern' };
          }
        }

        // Pipe-separated required patterns (all must be present)
        const patterns = expected.split('|').map((p) => p.trim().toLowerCase());
        const codeLC = userCode.toLowerCase();
        const results = patterns.map((pattern) => codeLC.includes(pattern));
        const allPresent = results.every(Boolean);
        const missing = patterns.filter((_, i) => !results[i]);
          ...
          <div className="max-w-3xl mx-auto p-6 lg:p-8">
            ...
            {(lesson.type === 'content' || lesson.type === 'video') && (
              <>
                ...
                <MarkdownRenderer content={getLessonContent(lesson.id)} />

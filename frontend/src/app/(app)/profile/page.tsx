...
import { useXPToken } from '@/hooks/use-xp-token';
...
import { TRACK_INFO, XP_CONFIG, getLevelTitle, DIFFICULTY_CONFIG, SOLANA_CONFIG } from '@/config/constants';
...
export default function ProfilePage() {
  ...
  const { xpToken, onChainXP, onChainLevel } = useXPToken();
  const { credentials, loading: credLoading, hasCredentials } = useCredentials();
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
                ...
                {connected && publicKey && (
                  <a
                    href={`https://explorer.solana.com/address/${publicKey}?cluster=${SOLANA_CONFIG.network}`}
                    target="_blank"
                ...
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  {balance !== null && (
                    <Badge variant="outline" className="text-xs border-[#14F195]/30 text-[#14F195]">
                      {balance.toFixed(4)} SOL on {SOLANA_CONFIG.network}
                    </Badge>
                  )}
                  {onChainXP !== null && (
                    <Badge variant="outline" className="text-xs border-quest-gold/30 text-quest-gold">
                      {onChainXP.toLocaleString()} on-chain XP (Token-2022)
                    </Badge>
                  )}
                  {onChainLevel !== null && (
                    <Badge variant="outline" className="text-xs border-[#9945FF]/30 text-[#9945FF]">
                      On-chain Level {onChainLevel}
                    </Badge>
                  )}
                </div>
              )}
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-8">
        <Tabs defaultValue="overview">
          ...
          <TabsContent value="credentials">
            {/* On-Chain XP Token Info */}
            {connected && xpToken && (
              <Card className="border-border/50 mb-6 bg-gradient-to-r from-[#9945FF]/5 to-[#14F195]/5">
                <CardContent className="p-5">
                  <h3 className="font-bold text-sm mb-3 flex items-center gap-2">
                    <Zap className="h-4 w-4 text-quest-gold" />
                    On-Chain XP Token (Token-2022 Soulbound)
                  </h3>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                    <div>
                      <p className="text-xs text-muted-foreground">Balance</p>
                      <p className="font-bold text-quest-gold">{xpToken.balance.toLocaleString()} XP</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">On-Chain Level</p>
                      <p className="font-bold">{xpToken.level}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Mint</p>
                      <a
                        href={`https://explorer.solana.com/address/${xpToken.mint}?cluster=${SOLANA_CONFIG.network}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="font-mono text-xs text-primary hover:underline flex items-center gap-1"
                      >
                        {xpToken.mint.slice(0, 6)}...{xpToken.mint.slice(-4)}
                        <ExternalLink className="h-3 w-3" />
                      </a>
                    </div>
                    <div>
                      ...
                      <a
                        href={`https://explorer.solana.com/address/${xpToken.tokenAccount}?cluster=${SOLANA_CONFIG.network}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="font-mono text-xs text-primary hover:underline flex items-center gap-1"
                      >
                        {xpToken.tokenAccount.slice(0, 6)}...{xpToken.tokenAccount.slice(-4)}
                        <ExternalLink className="h-3 w-3" />
                      </a>
                    </div>
                  </div>
              ...
              <div className="mb-6">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {credentials.map((cred) => {
                    const trackInfo = TRACK_INFO[cred.track as keyof typeof TRACK_INFO];
                    return (
                      <Card key={cred.id} className="border-border/50 glow-purple">
                        <CardContent className="p-5 text-center">
                          <div
                            className="w-24 h-24 mx-auto rounded-xl flex items-center justify-center mb-3 border"
                            style={{
                              backgroundColor: `${trackInfo?.color || '#9945FF'}15`,
                              borderColor: `${trackInfo?.color || '#9945FF'}30`,
                            }}
                          >
                            {cred.image ? (
                              <img src={cred.image} alt={cred.name} className="w-16 h-16 rounded-lg" />
                            ) : (
                              <span className="text-4xl">{trackInfo?.icon || '🏆'}</span>
                            )}
                          </div>
                          <h4 className="font-bold text-sm mb-1">{cred.name}</h4>
                          <p className="text-xs text-muted-foreground mb-2">
                            Level {cred.level} &bull; {cred.xp.toLocaleString()} XP
                          </p>
                          <Badge variant="outline" className="text-xs text-quest-health border-quest-health/30 mb-3">
                            <CheckCircle2 className="h-3 w-3 mr-1" />
                            {cred.compressed ? 'Compressed NFT' : 'Verified On-Chain'}
                          </Badge>
            ...
            {credLoading && (
              <div className="text-center py-8 text-muted-foreground text-sm">
                Scanning chain for credentials...
              </div>
            )}

            {/* Demo / Mock Credentials (always shown as examples) */}
            <h3 className="font-bold text-sm mb-3 mt-4">
              {hasCredentials ? 'Track Credentials' : 'Your Credentials'}
            </h3>
